...
class RvAdapter : RecyclerView.Adapter<RvAdapter.WeatherItemViewHolder>() {
    ...
    inner class WeatherItemViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(data: DayForecastModel) {
            ...
            itemView.findViewById<TextView>(R.id.tvTempMin).text =
                data.apparentTemperatureMin.toString() + '°'
            itemView.findViewById<TextView>(R.id.tvTempMax).text =
                data.apparentTemperatureMax.toString() + '°'
            itemView.findViewById<TextView>(R.id.tvPrecipitation).text =
                data.precipitationSum.toString() + "mm"
            itemView.findViewById<TextView>(R.id.tvWind).text =
                data.windspeed10mMax.toString() + "km/h"
        }
    }

    fun setForecast(weatherModel: ForecastModel?) {
        weatherList?.clear()
